...
import { UploadCloud, Server, FileImage, ShieldAlert, BookOpen, Trash2, Search, RefreshCw, PlusCircle, Layers } from 'lucide-react';
...
export const Admin: React.FC = () => {
  ...
  const { user } = useAuth();
  const { bookSeries, books, chapters, publishBook, deleteBookSeries, toggleSeriesStatus } = useUser();
    ...
    return (
      <div className={styles.admin}>
        <div className="main-container">
          <div className={`${styles.accessCard} glass`}>
            <ShieldAlert size={48} style={{ color: '#ef4444' }} />
            <h3>Access Restricted: Admin Credentials Required</h3>
            <p className={styles.subtitle}>
              You must be signed in with an Admin or Creator account to view the Admin Management Portal.
            </p>
            <div style={{ display: 'flex', gap: '12px', marginTop: '16px' }}>
              ...
              <button 
                ...
                style={{ padding: '10px 20px', fontSize: '13px', margin: 0 }}
              >
                Sign In as Admin
              </button>
            </div>
  ...
  const filteredCatalog = bookSeries.filter(s => 
    s.title.toLowerCase().includes(catalogSearch.toLowerCase()) ||
    s.status.toLowerCase().includes(catalogSearch.toLowerCase())
  );
  ...
  return (
    <div className={styles.admin}>
      <div className="main-container">
        ...
        <p className={styles.subtitle}>Upload, compile, and manage digital manga titles and catalog database entries.</p>
        ...
        <div className={styles.tabHeader}>
          ...
          >
            <PlusCircle size={16} /> Publish & DRM Compiler
          </button>
          <button 
            className={`${styles.tabBtn} ${activeTab === 'MANAGE' ? styles.tabBtnActive : ''}`}
            onClick={() => setActiveTab('MANAGE')}
          >
            <Layers size={16} /> Book Catalog Manager ({bookSeries.length})
          </button>
        </div>
        ...
        {activeTab === 'PUBLISH' ? (
            ...
            <div className={`${styles.consoleCard} glass`}>
              ...
              <div className={styles.logsTerminal}>
                <div className={styles.terminalHeader}>
                  <Server size={14} /> <span>DRM SECURE PIPELINE MONITOR</span>
                </div>
          ...
          /* TAB 2: Book Catalog Manager */
          <div className={`${styles.manageCard} glass`}>
            <div className={styles.searchBarWrapper}>
              <Search size={18} style={{ color: 'var(--text-muted)' }} />
              <input 
                type="text"
                placeholder="Search catalog by title or status..."
                value={catalogSearch}
                  ...
                  {filteredCatalog.length > 0 ? (
                      ...
                      return (
                        <tr key={series.id}>
                          ...
                          <td>
                            ...
                            <div style={{ fontSize: '11px', color: 'var(--text-muted)' }}>ID: {series.id}</div>
                          </td>
                          <td>
                            <span className={`${styles.statusBadge} ${series.status === 'ONGOING' ? styles.statusOngoing : styles.statusCompleted}`}>
                              {series.status}
                            </span>
                          </td>
                          <td>{seriesBooks.length} Volume(s)</td>
                          <td>
                            <button 
                              className={styles.btnAction}
                              onClick={() => toggleSeriesStatus(series.id)}
                            >
                              <RefreshCw size={12} style={{ display: 'inline', marginRight: 4 }} /> Toggle Status
                            </button>
                            <button 
                              className={`${styles.btnAction} ${styles.btnDelete}`}
                              onClick={() => {
                                if (window.confirm(`Are you sure you want to delete "${series.title}" from the catalog?`)) {
                                  deleteBookSeries(series.id);
                                }
                              }}
                            >
                    ...
                    <tr>
                      <td colSpan={5} style={{ textAlign: 'center', padding: '24px', color: 'var(--text-muted)' }}>
                        No manga series found in catalog matching search.
                      </td>
                    </tr>
                  )}
                </tbody>
